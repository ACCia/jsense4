...
#define S4_GET_LICENSE                          0x0000002e                      /** get the license */ 
#define S4_FREE_LICENSE                         0x0000002f                      /** free the license */ 
#define S4_RESET_LICENSE                        0x00000033                      /** Reset the license */ 
#define S4_SET_NET_CONFIG                       0x00000030                      /** set netlock config */
#define S4_GET_NET_CONFIG                       0x00000034                      /** get netlock config */
#define S4_GET_TOTAL_LICENSE					0x00000035						/** get total license of device, note: license'll be reset after invoking this control code */
//@}
...
typedef struct {
        ...
        BYTE                    reserve[RESERVE_NUM];                           /** reserved*/
...
typedef struct _S4NETCONFIG {
  unsigned int			dwLicenseMode;                                                    /* License Mode */
  unsigned int			dwModuleCount;                                                    /* Module Count */
  S4MODULEINFO  ModuleInfo[MAX_MODULE_COUNT];                                        /* Module detail infomation */        
} S4NETCONFIG;
...
DWORD WINAPI S4Test(char* p)
{
   //memset(p,0,200);
    ...
    /* malloc memory for the pointer to EliteIV context(s) */
    ...
    btDeviceNumber = dwCtxListSize/sizeof(SENSE4_CONTEXT);
        ...
        {
            /* Call S4Open(...) to open a EliteIV device in "share mode" according to the context pointer */
            ...
            /*
                Call S4Control(...,S4_GET_CUSTOMER_NAME,...) to get the Customer name of the currently opened
                EliteIV device .For more information about S4Control(...),please refer to the
                "EliteIV user Manual".
            */
            //dwResult = S4Control(pstS4CtxList+i, S4_GET_CUSTOMER_NAME, NULL, 0, lpOutBuf, sizeof(lpOutBuf), &dwBytesReturned);
            dwResult = S4Control(pstS4CtxList+i, S4_GET_SERIAL_NUMBER, NULL, 0, lpOutBuf, sizeof(lpOutBuf), &dwBytesReturned);
            if (S4_SUCCESS != dwResult)
            {
            ...
            }else
            {
                char buffer[200];
                int j = 0;
                int k = sprintf(buffer,"The Customer ID for device No.%d is:\n [",i);
                for(j=0; j<8; j++)
                {
                    k += sprintf(buffer+k,"0x%.2x ",lpOutBuf[j]);
                }
                k += sprintf(buffer+k,"]\n");
            ...
            /* Call S4Close(...) to close the EliteIV device previously opened. */
            dwResult = S4Close(pstS4CtxList+i);
            if (S4_SUCCESS != dwResult)
            {
